...
function buscarInforme() {
    ...
    var url = getUrl() + "informes/ventas?tipo=" + tipo + "&desde=" + desdeYmd + "&hasta=" + hastaYmd;
...
function buscarEstadisticas(tipo, desde, hasta) {
    ...
    var url = getUrl() + 'informes/gerencial/ventas/estadisticas'
        ...
        if (sec.filtros && sec.filtros.length) {
            ...
            sec.filtros.forEach(function (f) {
                ...
                filtrosHtml += '<div class="col-sm-3">'
                             ...
                             + '<select class="form-control input-sm" id="flt_' + idx + '_' + f.id + '">'
                             + opts + '</select></div>';
            });
...
function actualizarSeccion(idx, secId) {
    ...
    $.ajax({
        url: getUrl() + 'informes/gerencial/ventas/estadisticas/seccion?' + params,
        method: 'GET',
        success: function (resp) {
