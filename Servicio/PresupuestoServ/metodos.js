    ...
    .done(function (resultado) {
        ...
        for (let rs of resultado) {
            lista += "<tr class='item-list' onclick=\"seleccionPresupuesto("
                ...
                + rs.clientes_id + ","
                + rs.tipo_servicio_id + ","
                + (rs.promociones_cab_id || 0) + ","
                + (rs.descuentos_cab_id || 0) + ", '"
...
function seleccionPresupuesto(
    ...
    promociones_cab_id, descuentos_cab_id, 
...
function grabar() {
    ...
    if (!observaciones || !fecha || !fechaVence || empresa <= 0 || sucursal <= 0 || diagnostico <= 0) {
        swal({
            title: "Error",
    ...
    var endpoint = "presupuestoservcab/create";
    var metodo = "POST";
