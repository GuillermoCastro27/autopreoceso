    ...
    .done(function(resultado){
        ...
        for (let rs of resultado) {
            lista += "<li class=\"list-group-item\" onclick=\"seleccionProducto("
                + rs.item_id + ",'"
                + esc(rs.item_decripcion) + "',"
                + rs.tipo_impuesto_id + ",'"
                + rs.item_costo + "','"
                + rs.tip_imp_nom + "',"
                + rs.tipo_imp_tasa + ","
                + (rs.cantidad_disponible || 0) + ")\">"
                + esc(rs.item_decripcion) + " (Stock: " + (rs.cantidad_disponible || 0) + ")</li>";
        }
        lista += "</ul>";
...
function seleccionReclamoDet(item_id, item_decripcion, rec_cli_det_cantidad, rec_cli_det_cantidad_stock, rec_cli_det_costo, tipo_impuesto_id, tip_imp_nom, marca_id, modelo_id) {
